...
{
    [Authorize]
    public class SignSportsController : Controller
    {
        private readonly ApplicationDbContext _context;
        private readonly UserManager<ApplicationUser> _userManager;

        public SignSportsController(ApplicationDbContext context, UserManager<ApplicationUser> userManager)
        {
            _context = context;
        ...
        public async Task<IActionResult> Index()
        {
            ...
            return View(await _context.SignSports.Where(m => m.FactoryId == loggedInUser.FactoryId).ToListAsync());
        }

        // GET: SignSports/Details/5
        ...
        //    var signSport = await _context.SignSports
        ...
        //    return View(signSport);
        //}

        // GET: SignSports/Create
        [Authorize(Roles = "SuperAdmin,KSIAdmin,YSSAdmin,MAFAdmin,RFLAdmin,APEXAdmin,EDISONAdmin," +
        ...
        // POST: SignSports/Create
        [HttpPost]
        [ValidateAntiForgeryToken]
        [Authorize(Roles = "SuperAdmin,KSIAdmin,YSSAdmin,MAFAdmin,RFLAdmin,APEXAdmin,EDISONAdmin," +
            "KSIUserOfArchive,YSSUserOfArchive,MAFUserOfArchive,RFLUserOfArchive,APEXUserOfArchive,EDISONUserOfArchive")]
        public async Task<IActionResult> Create([Bind("Id,Name")] SignSport signSport)
        {
            if (ModelState.IsValid)
            {
                var loggedInUser = await _userManager.FindByNameAsync(HttpContext.User.Identity.Name);
                signSport.FactoryId = loggedInUser.FactoryId;
                _context.Add(signSport);
        ...
        public async Task<IActionResult> Edit(int? id)
        {
            ...
            var signSport = await _context.SignSports.FirstOrDefaultAsync(m => m.Id == id);
            if (signSport == null)
            {
                return NotFound();
            }
        ...
        // POST: SignSports/Edit/5
        [HttpPost]
        [ValidateAntiForgeryToken]
        [Authorize(Roles = "SuperAdmin,KSIAdmin,YSSAdmin,MAFAdmin,RFLAdmin,APEXAdmin,EDISONAdmin," +
            "KSIUserOfArchive,YSSUserOfArchive,MAFUserOfArchive,RFLUserOfArchive,APEXUserOfArchive,EDISONUserOfArchive")]
        public async Task<IActionResult> Edit(int id, [Bind("Id,FactoryId,Name")] SignSport signSport)
        {
            if (id != signSport.Id)
            {
                return NotFound();
            ...
            if (ModelState.IsValid)
            {
                try
                {
                    _context.Update(signSport);
                    await _context.SaveChangesAsync();
                }
                catch (DbUpdateConcurrencyException)
                {
                    if (!SignSportExists(signSport.Id))
                    {
                        return NotFound();
                    }
                    else
        ...
        // POST: SignSports/Delete/5
        [Authorize(Roles = "SuperAdmin,KSIAdmin,YSSAdmin,MAFAdmin,RFLAdmin,APEXAdmin,EDISONAdmin")]
        [HttpPost, ActionName("Delete")]
        [ValidateAntiForgeryToken]
        public async Task<IActionResult> DeleteConfirmed(int id)
        {
            var signSport = await _context.SignSports.FindAsync(id);
            _context.SignSports.Remove(signSport);
            await _context.SaveChangesAsync();
            return RedirectToAction(nameof(Index));
        }

        private bool SignSportExists(int id)
        {
            return _context.SignSports.Any(e => e.Id == id);
        }
        ...
        public async Task<IActionResult> LoadTable([FromBody]DTParameters dtParameters)
        {
            ...
            //here
            var loggedInUser = await _userManager.FindByNameAsync(HttpContext.User.Identity.Name);
            var result = await _context.SignSports.Where(m => m.FactoryId == loggedInUser.FactoryId).ToListAsync();
            ...
            //here
            var totalResultsCount = await _context.SignSports.Where(m => m.FactoryId == loggedInUser.FactoryId).CountAsync();
